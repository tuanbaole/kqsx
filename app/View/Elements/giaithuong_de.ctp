	...
	<thead>
		<tr>
			...
			<th class="text-center">
				<?php echo $this->Html->link(__('Xóa Hết'), array('controller'=>'giai_thuongs','action'=>'delete_all'), array('escape'=>false,'class' => 'delete_all_giai_thuong')); ?>
			</th>
		</tr>
	</thead>
	<tbody>
		...
		<?php foreach ($giai_thuong_des as $key_giai_thuong => $value_giai_thuong): ?>
			<tr class="<?php echo ($value_giai_thuong['Giaithuong']['so_nhay'] == 0)? 'info' : 'danger'; ?>" >
				...
				<td class="text-center">
					<i class="glyphicon glyphicon-remove text-danger cursor remove-giai-thuong" gt-id="<?php echo $value_giai_thuong['Giaithuong']['id']; ?>"></i>
				</td>
			</tr>
		<?php endforeach ?>
		<tr class="">
